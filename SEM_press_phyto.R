...
final_recovery <- lmer(final_stab ~ mean_Temp + (1|Lake), data=dataset_phyto_press)
...
# !!! always error message with singular fit !!!
...
# testing significance of all variables individually
biovolume <- lmer(mean_biomass_log ~ zoopl_body_size + (1|Lake), data=dataset_phyto_press)
...
# final_recovery <- lmer(final_stab ~ mean_Temp + (1|Lake), data=dataset_phyto_press)
# taking final recovery out of model, because it only shows correlations with singular fit
resilience <- lmer(rate_change_oti_trans ~ mean_Temp + (1|Lake),data=dataset_phyto_press)
AUC <- lmer(total_impact_log ~  mean_Temp + mean_ENS_D + (1|Lake), data=dataset_phyto_press)
biovolume <- lmer(mean_biomass_log ~ mean_Chla_log + (1|Lake), data=dataset_phyto_press)
ENS <- lmer(mean_ENS_D ~  mean_DOC_sqrt + mean_PAR_log + mean_Chla_log + (1|Lake), data=dataset_phyto_press)

dataset_phyto_press[is.na(dataset_phyto_press)] = 0
...
modelList <- psem(
  ...
  biovolume,
  ENS,  
  total_impact_log %~~% rate_change_oti_trans,
  mean_biomass_log  %~~% mean_ENS_D,
  dataset_phyto_press
  )
