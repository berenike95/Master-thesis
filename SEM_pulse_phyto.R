...
resistance <- lmer(initial_stab_sqrt ~ mean_J + (1|Lake), data=dataset_phyto_pulse)
summary(resistance)
...
resistance <- lmer(initial_stab_sqrt ~ mean_PAR_log + mean_zoopl_biomass_log + (1|Lake), data=dataset_phyto_pulse)
summary(resistance)
...
# testing significance of all variables individually 
recovery <- lmer(final_stab_sqrt ~ mean_TP + (1|Lake), data=dataset_phyto_pulse)
summary(recovery)
...
# --> PAR, Temp, zoopl biomass

# setting up the model fit 
recovery <- lmer(final_stab_sqrt ~  mean_zoopl_biomass_log + (1|Lake), data=dataset_phyto_pulse)
...
#---------------------------------------------------------------------------------------------------------#
# response = Chl a 
# explanatory = CTP, TN, Temp, PAR, DOC, mean zoopl biomass

Chla <- lmer(mean_Chla_log ~ mean_TP + (1|Lake), data=dataset_phyto_pulse)
summary(Chla)
# TP p-value = 0.00284 !
# TN p-value = 0.00439 !
# Temp p-value= 0.313
...
recovery <- lmer(final_stab_sqrt ~ mean_zoopl_biomass_log + mean_Temp + (1|Lake), data=dataset_phyto_pulse)
Chla <- lmer(mean_Chla_log ~ mean_DOC_sqrt + mean_TN + mean_Temp + mean_zoopl_biomass_log + (1|Lake), data=dataset_phyto_pulse)
...
modelList <- psem(
  ...
  ENS,
  Chla,
  mean_Chla_log %~~% mean_ENS_D,
  dataset_phyto_pulse
)
#  %~~% für ungerichtete Korrelationen

results <- summary(modelList)
# excluding resilience, biovolume and AUC, because of singular fit!
# exluding richness, evenness 
