...
resistance <- lmer(initial_stab_sqrt ~ mean_zoopl_biomass_log + mean_PAR_log + (1|Lake), data=dataset_phyto_pulse)
summary(resistance)
...
test <- lm(initial_stab_sqrt ~ initial_zoop_body_size_log + mean_zoopl_biomass_log + mean_biomass_log + mean_ENS_D + mean_DOC_sqrt + mean_TN_TP_ratio_100 + mean_DN_TP_ratio_100 + mean_PAR_log + mean_TN + mean_Temp, data=dataset_phyto_pulse)
test <- lm(initial_stab_sqrt ~ initial_zoop_body_size_log + mean_zoopl_biomass_log + mean_biomass_log + mean_J + mean_ENS_D + mean_TN_TP_ratio_100 + mean_PAR_log + mean_TN + mean_Temp, data=dataset_phyto_pulse)
test <- lm(initial_stab_sqrt ~ initial_zoop_body_size_log + mean_zoopl_biomass_log + mean_biomass_log + mean_J + mean_ENS_D + mean_DOC_sqrt + mean_s_sqrt +mean_TN_TP_ratio_100 + mean_DN_TP_ratio_100 + mean_PAR_log + mean_TN + mean_Temp, data=dataset_phyto_pulse)
test <- lm(initial_stab_sqrt ~ initial_zoop_body_size_log + mean_zoopl_biomass_log + mean_biomass_log + mean_J + mean_ENS_D + mean_DOC_sqrt + mean_s_sqrt +mean_TN_TP_ratio_100 + mean_DN_TP_ratio_100 + mean_PAR_log + mean_TN + mean_Temp, data=dataset_phyto_pulse)

vif(test)
#  mean_PAR_log       mean_zoopl_biomass_log 
# 1.015549               1.015549 
...
# testing significance of all variables individually 
recovery <- lmer(final_stab_sqrt ~ initial_zoop_body_size_log + (1|Lake), data=dataset_phyto_pulse)
summary(recovery)
...
ggplot(dataset_phyto_pulse, aes(mean_zoopl_biomass, initial_zoop_body_size)) +
  geom_point() +
...
# setting up the model fit 
# excluding mean_Temp and initial_zoop_body_size_log
...
# testing significance of all variables individually 
AUC <- lmer(total_impact_log ~ initial_zoop_body_size_log + (1|Lake), data=dataset_phyto_pulse)
summary(AUC)
...
AUC <- lmer(total_impact_log ~ mean_Chla_log + mean_Temp + (1|Lake), data=dataset_phyto_pulse)

summary(AUC)

r.squaredGLMM(AUC)
# R2m       R2c
# [1,] 0.3289206 0.4322132
...
# testing significance of all variables individually 
biovolume <- lmer(mean_biomass_log ~ initial_zoop_body_size_log + (1|Lake), data=dataset_phyto_pulse)
summary(biovolume)
...
# setting up the model fit 
biomass <- lmer(mean_biomass_log ~  mean_ENS_D + mean_TN_TP_ratio_100 + mean_Temp + (1|Lake), data=dataset_phyto_pulse)

summary(biomass)

r.squaredGLMM(biomass)
# R2m       R2c
# [1,] 0.5005218 0.6651982
...
ENS <- lmer(mean_ENS_D ~  mean_PAR_log + mean_TN + (1|Lake), data=dataset_phyto_pulse)
biomass <- lmer(mean_biomass_log ~  mean_TN_TP_ratio_100 + mean_Temp + (1|Lake), data=dataset_phyto_pulse)
resilience <- lmer(rate_change_ort ~ mean_PAR_log + mean_zoopl_biomass_log + (1|Lake), data=dataset_phyto_pulse)
resistance <- lmer(initial_stab_sqrt ~ mean_zoopl_biomass_log + (1|Lake), data=dataset_phyto_pulse)
recovery <- lmer(final_stab_sqrt ~ mean_PAR_log + initial_zoop_body_size_log + (1|Lake), data=dataset_phyto_pulse)
...
modelList <- psem(
  ...
  biomass,
  ENS,
  final_stab_sqrt %~~% rate_change_ort,
  mean_ENS_D %~~% mean_biomass_log,
  dataset_phyto_pulse
)
