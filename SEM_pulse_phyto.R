...
resistance <- lmer(initial_stab_sqrt ~ initial_zoop_body_size_log + (1|Lake), data=dataset_phyto_pulse)
summary(resistance)
...
test <- lm(initial_stab_sqrt ~ mean_PAR_log + mean_zoopl_biomass_log, data=dataset_phyto_pulse)
vif(test)
...
# setting up the model fit 
biomass <- lmer(mean_biomass_log ~  mean_Chla_log + mean_Temp + (1|Lake), data=dataset_phyto_pulse)
...
# setting up the model fit 
ENS <- lmer(mean_ENS_D ~  mean_DN_TP_ratio_100 + (1|Lake), data=dataset_phyto_pulse)
