...
#---------------------------------------------------------------------------------------------------------#
# Chl as proxy for phytoplankton biomass 
Chla <- lmer(mean_Chla_log ~ zoopl_body_size + (1|Lake), data=phyto_pulse_press)
summary(Chla)
# TP p-value = 0.00249 !
# TN p-value = 3.25e-05 !
# TN/TP p-value = 0.1344  
# DN/TP p-value = 7.11e-07 !
# DOC p-value = 2.43e-06  !
# PAR p-value = 4.87e-06 !
# Temp p-value = 0.421 
# zoopl biomass p-value = 0.00243 !
# zoopl body size p-value = 0.702
...
resistance <- lmer(initial_stab ~ mean_PAR + zoopl_body_size + (1|Lake), data=phyto_pulse_press)
AUC <- lmer(total_impact ~ mean_PAR + (1|Lake), data=phyto_pulse_press)
biomass <- lmer(mean_biomass_log ~ mean_Chla_log + mean_TP_log + mean_Temp + mean_DOC + 
                  (1|Lake), data=phyto_pulse_press)
evenness <- lmer(mean_J ~ mean_Chla_log + mean_zoopl_biomass_sqrt + (1|Lake), data=phyto_pulse_press)
ENS <- lmer(mean_ENS_D_log ~ mean_zoopl_biomass_sqrt + mean_Chla_log + mean_PAR + (1|Lake), data=phyto_pulse_press)
Chla <- lmer(mean_Chla_log ~ mean_TP_log + mean_DOC + mean_PAR + (1|Lake), data=phyto_pulse_press)
