...
#  p-value = 1.406e-05
dataset_zooplankton_pulse$mean_TN_10 <- dataset_zooplankton_pulse$mean_TN*10
hist(dataset_zooplankton_pulse$mean_TN_10)
...
#  p-value = 0.002989
dataset_zooplankton_pulse$mean_DOC_10 <- dataset_zooplankton_pulse$mean_DOC/10
hist(dataset_zooplankton_pulse$mean_DOC_10)
...
# p-value = 3.626e-06
dataset_zooplankton_pulse$mean_Temp_10 <- dataset_zooplankton_pulse$mean_Temp/10
hist(dataset_zooplankton_pulse$mean_Temp_10)
...
# p-value = 0.001136
dataset_zooplankton_pulse$mean_J_10 <- dataset_zooplankton_pulse$mean_J*10
hist(dataset_zooplankton_pulse$mean_J_10)
...
test <- lm(initial_stab_log ~ mean_DOC_sqrt + mean_biomass_log + mean_J + mean_s, 
...
resistance <- lmer(initial_stab_log ~ mean_s + (1|Lake), data=dataset_zooplankton_pulse)

summary(resistance)
# only richness significant?

r.squaredGLMM(resistance)
#    R2m       R2c
# [1,] 0.2545625 0.6770053
...
recovery <- lmer(final_stab_sqrt ~ mean_Temp_10 + (1|Lake), data=dataset_zooplankton_pulse)
...
resilience <- lmer(rate_change_ort ~ mean_Temp_10 + (1|Lake), data=dataset_zooplankton_pulse)
...
test <- lm(total_impact_log ~ mean_TP_log + mean_TN_TP_ratio_100 + mean_DOC_10 + mean_Temp_10 + 
...
AUC <- lmer(total_impact_log ~ mean_DOC_10 + mean_biomass_log + (1|Lake), data=dataset_zooplankton_pulse)
...
biomass <- lmer(mean_biomass_log ~ mean_Temp_10 + mean_TN_10 + (1|Lake), data=dataset_zooplankton_pulse)
...
evenness <- lmer(mean_J ~ mean_Temp_10 + (1|Lake), data=dataset_zooplankton_pulse)
...
ENS <- lmer(mean_ENS_D ~ mean_DOC_10 + mean_PAR_log + (1|Lake), data=dataset_zooplankton_pulse)
...
modelList <- psem(resistance, recovery, resilience, AUC, biomass, evenness, ENS, dataset_zooplankton_pulse) 
